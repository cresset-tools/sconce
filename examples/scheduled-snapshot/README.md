...
| `upload-snapshot.sh` | `.github/scripts/upload-snapshot.sh` (`chmod +x`) |
...
sconce ci-policy add \
  ...
  --audience https://repo.example.com \
  --claim repository=acme/backend \
  --capability publish
```

- `--audience` must equal what the job requests (default: `SCONCE_URL`; override
  with `SCONCE_OIDC_AUDIENCE`).
...
### 2. In your app repo — secrets and files
...
- Commit your `shop.jibs` config (the tables + anonymization rules jibs runs
  server-side).
- Copy the two files above; edit the `env:` block in the workflow
...
- The uploader **chunks** the dump (`upload-snapshot.sh` splits it into
  `part_size_limit` pieces and completes with the whole-file sha256), so multi-GB
  dumps aren't bounded by a single request body limit. sconce assembles the parts
  and verifies the sha before registering the snapshot.
